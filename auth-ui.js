...
function ensureUi(){
  ...
  if(top&&!byId('authButton')){
    const badge=byId('bookBadge');
  ...
  const area=byId('collectionArea');
  if(area&&!byId('collectionAuth')){
  ...
  if(!byId('authModal')){
    ...
    byId('authGoogle').onclick=googleLogin;
    byId('authSignIn').onclick=emailLogin;
    byId('authSignUp').onclick=emailSignup;
    byId('authSignOut').onclick=logout;
  }
}
function identity(){return user?.email||user?.user_metadata?.full_name||'ログイン中'}
function openAuth(text=''){ensureUi();if(text)message=text;drawAuth();byId('authModal').classList.add('on');document.body.style.overflow='hidden'}
function closeAuth(){byId('authModal')?.classList.remove('on');document.body.style.overflow=''}
function toast(t,type='ok'){let x=byId('authToast');if(!x){x=document.createElement('div');x.id='authToast';x.className='authToast';document.body.appendChild(x)}x.textContent=t;x.dataset.type=type;x.classList.add('on');clearTimeout(x._t);x._t=setTimeout(()=>x.classList.remove('on'),2400)}
function drawAuth(){
  ensureUi();
  const authButton=byId('authButton'),collectionAuth=byId('collectionAuth');
  ...
  const c=byId('collectionLogin');if(c)c.onclick=()=>openAuth('いま発見済みの商品をログイン後の図鑑へ引き継ぎます。');
  byId('authOut').classList.toggle('authHidden',!!user);byId('authIn').classList.toggle('authHidden',!user);byId('authIdentity').textContent=identity();
  const msg=byId('authMessage');msg.textContent=message||(!configured?'Supabase接続設定待ち。ゲストプレイは利用できます。':user?'クラウド図鑑に接続済み。':'ゲストプレイ中。');msg.classList.toggle('warn',!configured);
  ['authGoogle','authSignIn','authSignUp'].forEach(id=>{const b=byId(id);if(b)b.disabled=busy||!configured});
}
...
render=function(){if(!mem.s)mem.s=new Set();originalRender();byId('bookBadge').textContent=(user?'図鑑 ':'仮図鑑 ')+mem.d.size+' / '+allItems.length;if(!user&&tab!=='d'){byId('collection').innerHTML='<div class="collectionGate"><b>ログイン後に使えます。</b><div class="sub">欲しい・持ってる・あとで見るはアカウントへ保存します。</div><button class="collectionLogin" id="gateLogin" type="button">ログイン</button></div>';byId('gateLogin').onclick=()=>openAuth('この機能はログイン後に保存できます。')}drawAuth()};
...
showLoot=function(x,boss){previousShowLoot(x,boss);const actions=document.querySelector('#lootState .lootActions');if(actions&&!byId('later')){const b=document.createElement('button');b.id='later';b.type='button';b.className='action '+(mem.s.has(x.id)?'on':'');b.textContent='🔖 あとで見る';b.onclick=()=>tog('s',x.id,b);actions.appendChild(b)}};

function creds(){return{email:String(byId('authEmail').value||'').trim(),password:String(byId('authPassword').value||'')}}
